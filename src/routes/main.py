...
@main_bp.route("/clientes_finalizados/download/<int:cliente_id>", methods=["GET"])
@login_required
def download_contrato(cliente_id):
    cliente = ClienteFinalizado.query.get_or_404(cliente_id)

    context = {
        "{{CLIENTE}}": cliente.cliente_nome,
        "{{CPF}}": cliente.cpf,
        "{{PROCESSO}}": cliente.numero_processo or "",
        "{{VARA}}": cliente.vara or "",
        "{{PRECO_AQUISICAO}}": f"R$ {cliente.preco_aquisicao:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if cliente.preco_aquisicao else "",
        "{{DATA_COMPRA}}": cliente.data_compra.strftime("%d/%m/%Y") if cliente.data_compra else "",
        "{{RESPONSAVEL}}": cliente.responsavel_id,
        # Adicione mais campos conforme seu modelo DOCX
    }

    template_path = os.path.join(current_app.root_path, "Contrato_OVERTIME_Modelo.docx")

    if not os.path.exists(template_path):
        current_app.logger.error(f"Modelo não encontrado: {template_path}")
        abort(404, description="Modelo de contrato não encontrado.")

    from src.contract_generator import generate_contract_docx
    from io import BytesIO

    contrato_stream = BytesIO()
    generate_contract_docx(template_path, contrato_stream, context)
    contrato_stream.seek(0)

    filename = f"Contrato_{cliente.cliente_nome.replace(' ', '_')}_{cliente.cpf}.docx"
