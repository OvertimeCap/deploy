...
def clientes_finalizados():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('main.download_contrato', cliente_id=novo_cliente.id))
...
def download_contrato(cliente_id):
    ...
    context = {
        ...
        "{{TELEFONE_RESPONSAVEL}}": cliente.telefone_responsavel or ""
    }

    template_path = os.path.join(current_app.root_path, "Contrato_OVERTIME_Modelo.docx")
    contrato_bytes = io.BytesIO()
    generate_contract_docx(template_path, contrato_bytes, context)
    contrato_bytes.seek(0)

    return send_file(
        contrato_bytes,
        mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        as_attachment=True,
        download_name=f"Contrato_{cliente.nome_cliente}.docx"
    )
